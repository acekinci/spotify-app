...
function FullScreenPlayer({toggle,state,controls, volumeIcon}){
    ...
    <div className="h-full relative"  onClick={controls[state?.playing ? 'pause' : 'play']}>
        ...
        <div className="absolute opacity-70 top-14 left-8 text-white flex items-center gap-x-4">
        ...
        <div className="absolute bottom-36 left-8 flex items-center gap-x-6 border-none">
         <img src={current.image} alt="" className="w-24 h-24 object-cover"/>
         <div className="self-end">
         <h3 className="text-3xl font-bold">{current.title}</h3>
         <p className="text-link text-sm mt-1">{current.description}</p>
         </div>
        </div>
        <div
        onClick={e =>{
        ...
        <div className="flex items-center gap-x-3 absolute bottom-4 right-6">
     <button 
     ...
     <div className="max-w-full w-[5.813rem]">
        ...
        onClick={toggle}
        className="w-8 h-8 flex items-center justify-center text-white text-opacity-70 hover:text-opacity-100">
        <Icon size={24} name="fullScreenRightOff" />
        </button>
        </div>
     </div>
    </div>
